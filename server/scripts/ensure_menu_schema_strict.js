...
async function run(){
  ...
  // degressive Defaults, wo noch keine Tiers existieren
  const prods = (await query(`SELECT id, price_cents FROM products WHERE active=1`,[])).rows||[]
  for (const p of prods){
    const n = (await query(`SELECT COUNT(1) n FROM product_price_tiers WHERE product_id=?`,[p.id])).rows?.[0]?.n||0
    if (n>0) continue
    const base = Number(p.price_cents||0); if(!base) continue
    const tiers = [
      { g: 1,  price: base },
      { g: 3.5, price: Math.round(base*3.5*0.96) },
