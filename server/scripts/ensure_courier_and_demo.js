...
async function seedDemo(){
  const openCnt = (await query(`SELECT COUNT(*) AS n FROM orders WHERE status='wartet_bestätigung'`,[])).rows?.[0]?.n || 0
  if (Number(openCnt)>0) return
  // aktives Produkt sicherstellen
...
await ensureSchema(); await seedDemo();
