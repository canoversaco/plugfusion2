...
// JWT "lesen" ohne verify, damit wir /orders benutzen können
...
// Kategorien (inkl. Highlight-Felder)
...
r.post('/orders', withAuth, async (req,res)=>{
  try{
    ...
    const prows = ids.length
      ? (await query(`SELECT id, name, price_cents FROM products WHERE active=1 AND id IN (${ph})`, ids)).rows || []
      : []
