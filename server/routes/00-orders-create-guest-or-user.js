...
async function one(sql, params){ try { return await db.one(sql, params); } catch(_){ let i=0; return db.one(sql.replace(/\?/g,()=>'$'+(++i)), params); } }
async function exec(sql, params){ try { return await db.exec(sql, params); } catch(_){ let i=0; return db.exec(sql.replace(/\?/g,()=>'$'+(++i)), params); } }

async function getGuestUserId(){
  // nutzt das Script, um 100%ig eine ID zu bekommen
  const { spawnSync } = require('child_process');
  const out = spawnSync(process.execPath, ['server/scripts/ensure_guest_user.js'], { encoding: 'utf8' });
  if (out.status === 0) {
    const id = Number(String(out.stdout || '').trim());
    if (id && isFinite(id)) return id;
  }
  // Fallback ohne child_process
  const row = await one('select id from users where username = ?', ['guest']).catch(()=>null);
...
router.post('/api/orders', express.json(), async (req, res) => {
  try {
    // 1) User ermitteln (JWT wenn vorhanden, sonst guest)
    let userId = null;
    try {
    ...
    } catch(_) { /* ungültiges Token -> weiter mit Gast */ }

    if (!userId) userId = await getGuestUserId();
    ...
    console.error('[orders-guest-or-user]', e);
