...
const { authzOptional } = require('../mw/authz');
const { db } = require('../db');
...
async function one(sql, params) {
  ...
  return db.one(pgSql, params);
}
async function exec(sql, params) {
  try { return await db.exec(sql, params); } catch(_) {}
  let i=0; const pgSql = sql.replace(/\?/g, ()=>'$'+(++i));
  return db.exec(pgSql, params);
}
...
function resolveUserId(req){
  // 1) Aus JWT (falls vorhanden)
  const jwtId = req.user && (req.user.id || req.user.user_id || req.user.uid || req.user.sub);
  if (jwtId) return Number(jwtId);
  // 2) Dev-Fallback: ENV erlaubt Guest-Bestellung mit user_id=1
  if (process.env.DEV_ALLOW_ORDER_WITHOUT_AUTH === '1' || process.env.ALLOW_ORDER_WITHOUT_AUTH === '1') {
    return 1;
  }
  return null;
}
...
// --- Route ---------------------------------------------------
router.post('/api/orders', authzOptional, async (req, res) => {
  try {
    const userId = resolveUserId(req);
    if (!userId) return res.status(401).json({ error: 'unauth', reason: 'no_user' });
