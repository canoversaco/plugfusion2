...
export default function AdminCoreTools() {
  ...
  async function setOrderStatus() {
    ...
    for (const t of tries) { try { await t(); setMsg("OK – Bestellstatus aktualisiert."); return; } catch{} }
    setMsg("Konnte Bestellstatus nicht setzen.");
  }

  return (
    <div className="space-y-4">
      <div className="p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 shadow-lg backdrop-blur">
        <div className="font-medium mb-2">Bestell-Tools</div>
        <div className="flex flex-wrap gap-2 items-center">
          <input value={orderId} onChange={e=>setOrderId(e.target.value)} placeholder="Order-ID" className="px-3 py-2 rounded-xl rounded bg-zinc-800 border border-zinc-700 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
          <select value={status} onChange={e=>setStatus(e.target.value)} className="px-3 py-2 rounded-xl rounded bg-zinc-800 border border-zinc-700 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">
          ...
          <input value={courier} onChange={e=>setCourier(e.target.value)} placeholder="Kurier-ID (optional)" className="px-3 py-2 rounded-xl rounded bg-zinc-800 border border-zinc-700 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
          <button onClick={setOrderStatus} className="px-3 py-2 rounded-xl rounded bg-violet-600 hover:bg-violet-500 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">Setzen</button>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 shadow-lg backdrop-blur">
        ...
        {(ui.highlights||[]).length===0 ? (
          <div className="text-zinc-400 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">Keine Highlights vorhanden.</div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {(ui.highlights||[]).map((h,i)=>(
              <div key={i} className="rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900/60">
                ...
                <div className="p-2">
                  <div className="font-medium text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">{h.title}</div>
